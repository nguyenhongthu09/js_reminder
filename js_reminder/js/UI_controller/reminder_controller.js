...
export const renderReminderonUI = (listNoteId) => {
  ...
  const trueReminders = filteredReminders.filter((reminder) => reminder.status);
  const falseReminders = filteredReminders.filter((reminder) => !reminder.status);
  ...
  cartReminder.innerHTML = `
    ...
    <!-- Render falseReminders -->
    ${renderReminders(falseReminders, false)}
    <!-- Render trueReminders -->
    ...
    .map((reminder) => {
      return `
        <div class="reminder__detail" id=${reminder.id} data-listnote-id=${reminder.idlist}>
          <div class="items-list-reminder">
            <div class="form-check item-reminder">
              <input class="form-check-input thuthu" type="checkbox" id-input=${reminder.id} ${
                reminder?.status ? "checked" : ""
              }>
              <input type="text" class="form-check-name ${status ? "checked" : ""}" data-reminder-id="${
        reminder.id
      }" value="${reminder.title}" placeholder="Add Note"></input>
            </div>
          </div>
...
const checks = (reminderInputs, reminderState) => {
  reminderInputs.forEach((input) => {
    input.addEventListener("click", async (event) => {
      ...
      const reminderDetail = event.target.parentNode.closest(".reminder__detail");
