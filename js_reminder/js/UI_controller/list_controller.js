        ...
        <div class="listnote" id=${list.id}>
          <div class="list-note" data-listId=${list.id}>
            <div class="item-list-none-left">
              <div class="icon-list-note" style="background-color: ${backgroundColor};">
            ...
            <div class="item-list-none-right">
              <span class="number-items-notes" id="total-done-${list.id}">${totalDone}</span>
              <span class="number-items-note" id="total-count-${list.id}">/${totalCount}</span>
            </div>
          </div>
          <div class="icon-home-list-del">
...
const updateListElement = (idlist, selector, value) => {
  const { totalDone, totalCount } = getListTotals(idlist);
  const element = document.querySelector(
    `.list-note[data-listId="${idlist}"] ${selector}`
  );

  if (element) {
    element.textContent =
      value === "totalDone" ? `${totalDone}` : `/${totalCount}`;
  }
  renderListOnUI("renderlist-home");
};
export const updateListCheckbox = async (idlist) => {
  updateListElement(idlist, "#total-done", "totalDone");
};

export const updateListTotalCount = async (idlist) => {
  updateListElement(idlist, "#total-count", "totalCount");
};

const deleteListNoteEvent = () => {
...
const handleEditList = (id, element) => {
  ...
  const updateButtonState = () => {
    btnSubEdit.disabled = editInputName.value.trim() === "";
  };

  editInputName.addEventListener("input", updateButtonState);
  if (listToEdit) {
