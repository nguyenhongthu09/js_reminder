...
const updateListElement = async (idlist, selector, value) => {
  const { totalDone, totalCount } = await getListTotals(idlist);
...
export const updateListCheckbox = async (idlist) => {
  await updateListElement(idlist, "#total-done", "totalDone");
};

export const updateListTotalCount = async (idlist) => {
  await updateListElement(idlist, "#total-count", "totalCount");
};
